...
namespace Tip.TransactionStandard.Contracts.Orders;

[XmlRoot("BuyerOrders", Namespace = TipXml.Namespace)]
public sealed class BuyerOrdersRequest : ITipValidatable
{
    [Required]
    ...
    [Required]
    [JsonPropertyName("orderBookedDate")]
    [XmlElement("orderBookedDate")]
    ...
    {
        if (!TipDateTime.IsDateLike(OrderBookedDate))
        {
            yield return new TipValidationIssue(path.Append(nameof(OrderBookedDate)), "orderBookedDate must be a valid ISO date.");
        }
    }
}

[XmlRoot("SellerOrders", Namespace = TipXml.Namespace)]
public sealed class SellerOrdersRequest
{
    [Required]
    [JsonPropertyName("transactionHeader")]
    [XmlElement("transactionHeader")]
